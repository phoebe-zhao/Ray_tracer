...
{
  //std::cout << "image is " << m_data[m_colorComponents * (m_width * y + x) + i] << std::endl;
  return m_data[m_colorComponents * (m_width * y + x) + i];
}
...
/*bool Image::loadPng(const std::string & filename){
  ...
  size_t size = m_width * m_height * m_colorComponents;
	...
	for (uint y(0); y < m_height; y++) {
		for (uint x(0); x < m_width; x++) {
			for (uint i(0); i < m_colorComponents; ++i) {
				...
				color = clamp(color, 0.0, 1.0);
				m_data[m_colorComponents * (m_width * y + x) + i] = color;
        //std::cout << "color is " << m_data[m_colorComponents * (m_width * y + x) + i] << std::endl;
			}
		}
	}
