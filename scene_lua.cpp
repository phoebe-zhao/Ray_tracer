...
extern "C"
int gr_node_transparent_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
  ...
  double shininess = luaL_checknumber(L, 3);
  bool transparent = lua_toboolean(L, 5);
  float ior = luaL_checknumber(L, 6);

  data->material = new PhongMaterial(glm::vec3(kd[0], kd[1], kd[2]),
                                     glm::vec3(ks[0], ks[1], ks[2]),
                                     shininess,transparent,ior);
...
extern "C"
int gr_node_set_intersec_cmd(lua_State* L){
  GRLUA_DEBUG_CALL;
  ...
  std::string another_node = luaL_checkstring(L, 2);
  self->set_intersection(another_node);
...
extern "C"
int gr_node_set_difference_cmd(lua_State* L){
  GRLUA_DEBUG_CALL;
  ...
  std::string another_node = luaL_checkstring(L, 2);
  self->set_difference(another_node);

  return 0;
}

extern "C"
int gr_node_set_glossy(lua_State* L){
  GRLUA_DEBUG_CALL;
  ...
  int glossy = luaL_checknumber(L, 2);
  self->set_glossy(glossy);
...
int gr_node_set_bump_cmd(lua_State* L){
  ...
  std::cout << "finish set texture" << std::endl;
...
static const luaL_Reg grlib_functions[] = {
  ...
  {"transparent",gr_node_transparent_cmd},
  {0, 0}
};
...
static const luaL_Reg grlib_node_methods[] = {
  ...
  {"set_intersection",gr_node_set_intersec_cmd},
  {"set_difference",gr_node_set_difference_cmd},
  {"set_bump",gr_node_set_bump_cmd},
  {"set_glossy",gr_node_set_glossy},
